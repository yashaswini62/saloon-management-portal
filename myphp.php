...
if(isset($_REQUEST['cod']))
						{
							extract($_REQUEST);
							$userid=$_SESSION['id'];
							$n=iud("INSERT INTO `bookings`(`user_id`, `goggle_id`, `way`, `Mobile`, `Landmark`, `city`) VALUES 
							('$userid','$goggleid','Cash On Delivery','$mobile','$landmark','$city')");
							if($n==1)
							{
...
if(isset($_REQUEST['booking']))
						{
							extract($_REQUEST);
							$userid=$_SESSION['id'];
							$n=iud("INSERT INTO `booking`( `userid`, `plan`, `price`, `time`) VALUES 
							('$userid','$plan','$price','$time')");
							if($n==1)
							{
								echo"<script>alert('Successful');
								window.location='index.php';
